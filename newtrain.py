...
def get_ds(config):
    ...
    text_pairs_prep = []
    for i in range(len(text_pairs[:512])):
        string=""
        sqam=prepro_squared_ampl(text_pairs[i][2])
        for k in sqam:
            string+=k
        text_pairs_prep.append({'id':i,'translation':{'interaction':text_pairs[i][0], 'graph':prepro_feyn(text_pairs[i][1]), 'sqAmp':string[:700], 't':text_pairs[i][3]}})
